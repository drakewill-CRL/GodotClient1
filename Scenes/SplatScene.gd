...
#This is the Splat client demo. Uses 2 layers of tiles instead of 1, but otherwise should be
#very similar to the overhead view scene test.

@onready var endpoints: PraxisEndpoints = $PraxisEndpoints
...
func _ready():
	...
	for x in range(tilesX / -2, (tilesX /2) + 1):
		for y in range(tilesY / -2, (tilesY / 2) + 1):
			...
			#Now add the splat tile.
			var splattile = mapTileScene.instantiate()
			#only the next 3 lines need adjusted from the above block
			splattile.name = "splatmapTile_" + str(x) + "_" + str(y) 
			splattile.styleSet = "splatter" 
			splattile.autoRefresh = true 
			splattile.customPath = '/Splatter/MapTile/'
			add_child(splattile)
			splattile.xOffset = x
			splattile.yOffset = y
			splattile.position.x = x * PraxisMapper.mapTileWidth + x + x
			splattile.position.y = -y * PraxisMapper.mapTileHeight - y -y
			PraxisCore.plusCode_changed.connect(splattile.OnPlusCodeChanged)
			splattile.GetTile(PraxisMapper.currentPlusCode)
			
	camera.position.x = startX
	camera.position.y = startY
	
	print("Splat scene loaded")
...
func CameraScroll(currentPlusCode, previousPlusCode):
	...
	endpoints.DemoSplatterEnter(currentPlusCode)
	
func TestSplat():
	print("splat testing")
	endpoints.DemoSplatterSplat(PraxisMapper.currentPlusCode, 3)
	
	#var api = await PraxisAPICall.new()
	#api.result.connect(TestSplatDone)
	#api.DemoSplatterTest(PraxisMapper.currentPlusCode.substr(0,8))
	
func TestSplatDone(event):
	print("splat done")
